...
const BountyItem = ({ setFilter, ...props}) =>
<Flex direction={['row']} align={"stretch"} justify="center" wrap='wrap' mb={30} >
  ...
  <Flex align="left" justify="left" w={[0.45]} pl={10} py={[7]} direction={['column']} of='hidden' >
    <Heading color={'white'} level={3} margin={'0'} display='inline-block' f={[3]} fontWeight={'700'}>
      <Link to={`bounty/${props.pk}`}>
        {idx(props, _=>_.title)}
      </Link>
    </Heading>
    <DescriptionSnippet>
...
return <div>
  ...
  {
    props.data.map(data=> <BountyItem {...data} /> )
  }
</div>
}